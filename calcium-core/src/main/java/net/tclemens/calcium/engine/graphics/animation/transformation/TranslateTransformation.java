...
/**
 * This class represents an interpolated translate transformation
 *
 * @author Tim Clemens
 */
final class TranslateTransformation extends Transformation {
     ...
     */
    TranslateTransformation(float x, float y, float z, Interpolation interpolation) {
    ...
    public final Matrix3D getMatrix() {
        ...
        return computeTranslate(x, y, z, interpolation);
    }
    ...
    public final Transformation update(long time) {
        ...
        if (interpolation.isDynamic()) {
            ...
            return TransformationFactory.buildTranslate(x, y, z, interpolation.update(time));
        }

        Matrix3D matrix = computeTranslate(x, y, z, interpolation);
    ...
    private static Matrix3D computeTranslate(float x, float y, float z, Interpolation interpolation) {

        float scale = interpolation.getValue();

        return MatrixFactory.createTranslate3D(scale * x, scale * y, scale * z);
    }
}
